...
for j=1:252
    try
        ...
        %g_C0 = results_array(ii).part_vals(:,1) + results_array(ii).part_vals(:,2).*(1./(1 + ...
        %    (100*ones(size(results_array(ii).part_vals(:,1)))).^(results_array(ii).part_vals(:,7))));
    ...
    %(results_array(ii).part_sim(:,1)+ results_array(ii).part_sim(:,T+1)+results_array(ii).part_sim(:,2*T+1)).^(results_array(ii).part_vals(:,7))));
        %g_CT = results_array(ii).part_vals(:,1) + results_array(ii).part_vals(:,2).*(1./(1 + ...
         %       (1000*ones(size(results_array(ii).part_vals(:,1)))).^(results_array(ii).part_vals(:,7))));
        ...
        %(results_array(ii).part_sim(:,T)+ results_array(ii).part_sim(:,2*T)+results_array(ii).part_sim(:,3*T)).^(results_array(ii).part_vals(:,7))));
...
J2 = find(ismember(iso_id,{'CHN','KOR','TWN','SGP','AUS'}));
J3 = find(ismember(iso_id,{'USA','ESP','DEU','ITA','CHE','IRN','FRA','GBR','TUR'}));
figure;
subplot(1,3,1)
...
xlabel('$\gamma$');ylabel('$n$');%xlim([0,1]);ylim([0,2]);
xlim([0,1]); ylim([0.4,1.8]);

%subplot(2,3,4)
%errorbar(point_ests(:,4),point_ests(:,7),CI.c95(:,7),CI.c95(:,7),CI.c95(:,4),CI.c95(:,4),'o','Marker','none','CapSize',0);
%hold on;
%errorbar(point_ests(J2,4),point_ests(J2,7),CI.c95(J2,7),CI.c95(J2,7),CI.c95(J2,4),CI.c95(J2,4),'og','CapSize',0);
%text(point_ests(J2,4),point_ests(J2,7),iso_id(J2));
%xlabel('$\gamma$');ylabel('$n$');%xlim([0,1]);ylim([0,2]);
%errorbar(point_ests(:,4),point_ests(:,4),CI.c95(:,7),CI.c95(:,7),CI.c95(:,4),CI.c95(:,4),'o','Marker','none','CapSize',0);
subplot(1,3,2)
...
xlabel('$\gamma$');ylabel('$\kappa$');%xlim([0,1]);ylim([0,2]);
xlim([0,1]); ylim([0,1.8]);

% subplot(2,3,5)
% errorbar(point_ests(:,4),point_ests(:,8),CI.c95(:,8),CI.c95(:,8),CI.c95(:,4),CI.c95(:,4),'o','Marker','none','CapSize',0);
% hold on;
% errorbar(point_ests(J2,4),point_ests(J2,8),CI.c95(J2,8),CI.c95(J2,8),CI.c95(J2,4),CI.c95(J2,4),'og','CapSize',0);
% text(point_ests(J2,4),point_ests(J2,8),iso_id(J2));
% xlabel('$\gamma$');ylabel('$\kappa$');%xlim([0,1]);ylim([0,2]);
...
xlabel('$n$');ylabel('$\kappa$');%xlim([0,2]);ylim([0,2]);
xlim([0.4,1.8]); ylim([0,1.8]);
% subplot(2,3,4)
% errorbar(point_ests(:,7),point_ests(:,5),CI.c95(:,11),CI.c95(:,11),CI.c95(:,7),CI.c95(:,7),'o','Marker','none','CapSize',0);
% hold on;
% errorbar(point_ests(J3,7),point_ests(J3,11),CI.c95(J3,11),CI.c95(J3,11),CI.c95(J3,7),CI.c95(J3,7),'or','CapSize',0);
% text(point_ests(J3,7),point_ests(J3,11),iso_id(J3));
% errorbar(point_ests(J2,7),point_ests(J2,11),CI.c95(J2,11),CI.c95(J2,11),CI.c95(J2,7),CI.c95(J2,7),'og','CapSize',0);
% text(point_ests(J2,7),point_ests(J2,11),iso_id(J2));
% xlabel('$n$');ylabel('$\delta/\beta$');
figure;
subplot(1,2,1)
...
%figure;
 ...
 xlabel('$(R_0-R_e)/R_0$');ylabel('$[f(C_0)-f(C_T)]/f(C_0)$');%xlim([0,1]);ylim([0,2]);
return
% subplot(2,3,6)
% errorbar(point_ests(:,7),point_ests(:,8),CI.c95(:,8),CI.c95(:,8),CI.c95(:,7),CI.c95(:,7),'o','Marker','none','CapSize',0);
% hold on;
% xlabel('$n$');ylabel('$\kappa$');%xlim([0,2]);ylim([0,2]);
%return
%%
...
%         title(sprintf('%s %s',names{ii},results.ISO3166alpha3))
         %figure(fid+1);
%         
         %[S,ax]= plotmatrixcorr(results.part_vals,lab,sprintf('%s %s',names{ii},results.ISO3166alpha3));
         %title(sprintf('%s %s',names{ii},results.ISO3166alpha3))
         %fcnCorrMatrixPlot(results.part_vals,lab,sprintf('%s %s',names{ii},results.ISO3166alpha3))
%         %title()
%         
%         ii = ii + 1;
%         title(sprintf('%s %s',results.name,results.ISO3166alpha3))
%         %% plot accepted samples of posterior predictve distribution used in the 
%         % ABC inference against data
%         T = length(results.data.C);
%         optsf = [];
%         optsf.handle = figure(fid+2);
%         optsf.line_width = 1;
%         optsf.alpha = 0.5;
%         optsf.error = 'credint';
